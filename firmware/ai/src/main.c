...
void camera_task(void *parameters) {
  ...
  while (true) {
    ...
    if (g_wifiClientConnected == 1) {
      ...
      // cpxInitRoute(CPX_T_GAP8, CPX_T_STM32, CPX_F_APP, &g_txPacket.route); //
      ...
      cpxPrintToConsole(LOG_TO_CRTP,
                        ...
                        "%d ms, CPX = %d ms\n",
                        captureTime, processTime, encodeTime, imgSize,
                        transferTime, cpxTime);
    } else {
      vTaskDelay(10);
    }
  }
...
void rx_task(void *parameters) {
  ...
  while (true) {
    ...
    switch (wifiCtrl->cmd) {
    case WIFI_CTRL_STATUS_WIFI_CONNECTED: // Not used in access point (I think)
      cpxPrintToConsole(LOG_TO_CRTP, "WiFi connected (%u.%u.%u.%u)\n",
                        wifiCtrl->data[0], wifiCtrl->data[1], wifiCtrl->data[2],
                        wifiCtrl->data[3]);
...
void run(void) {
  ...
  if (g_eventGroup == NULL) {
    cpxPrintToConsole(
        LOG_TO_CRTP,
        "Failed to create event group, insuficient FreeRTOS heap available\n");
    pmsis_exit(-1);
  }
  ...
  // LED_TASK
  xTask = xTaskCreate(led_task, "LED_TASK", configMINIMAL_STACK_SIZE * 2, NULL,
