...
// free to serve as tmp workspace for applyRowCorrection if needed.
PI_L2 unsigned char *g_scratch;

static volatile int g_skip_rem = WARMUP_FRAMES;

static CPXPacket_t g_tx_packet;
...
static uint32_t rowGrad(const unsigned char *buf, int r) {
...
static int detectSplitRow(const unsigned char *buf) {
    ...
    for (int r = lo; r <= hi; r++) {
        if (r == split) continue;
        if (rowGrad(buf, r) > max_diff / 2) return -1;
    }
...
static void applyRowCorrection(int split, unsigned char *tmp) {
...
static void imageCaptureDoneCb(void *arg) {
    (void)arg;
    xEventGroupSetBits(g_event_group, CAPTURE_DONE_BIT);
}
...
void ledTask(void *parameters) {
    ...
    const TickType_t x_delay = 500 / portTICK_PERIOD_MS;
...
void cameraTask(void *parameters) {
    ...
    cpxInitRoute(CPX_T_GAP8, CPX_T_WIFI_HOST, CPX_F_APP, &g_tx_packet.route);
    ...
    while (true) {
        ...
        pi_camera_capture_async(&g_camera, g_dma_buf, IMG_SIZE,
            pi_task_callback(&g_task, imageCaptureDoneCb, NULL));
        pi_camera_control(&g_camera, PI_CAMERA_CMD_START, 0);

        start = xTaskGetTickCount();
        EventBits_t bits = xEventGroupWaitBits(
            g_event_group, CAPTURE_DONE_BIT, pdTRUE, pdFALSE, pdMS_TO_TICKS(5000));
        capture_time = xTaskGetTickCount() - start;
        ...
        if (!(bits & CAPTURE_DONE_BIT)) {
            cpxPrintToConsole(LOG_TO_CRTP,
                ...
                capture_time, frame_ok, frame_tx);
            if (setupCamera(&g_camera)) {
        ...
        if (g_skip_rem > 0) {
            ...
            cpxPrintToConsole(LOG_TO_CRTP, "[INFO] Warmup %d/%d cap=%ums\n",
                              WARMUP_FRAMES - g_skip_rem, WARMUP_FRAMES, capture_time);
            if (g_skip_rem == 0)
        ...
        int split = detectSplitRow(g_scratch);
        if (split >= 0) {
            applyRowCorrection(split, g_dma_buf);
        ...
        uint32_t tx_start = xTaskGetTickCount();
#if defined(JPEG_ENCODING)
        jpeg_encoder_process(&g_jpeg_encoder, &g_jpeg_buffer, &g_jpeg_data,
                             &jpeg_size);
        tx_start = xTaskGetTickCount();
        img_size = g_jpeg_header_size + jpeg_size + g_jpeg_footer_size;
        transferJpegImage(&g_tx_packet, img_size, g_jpeg_data.data, jpeg_size,
                          g_jpeg_header.data, g_jpeg_header_size,
                          g_jpeg_footer.data, g_jpeg_footer_size);
#elif defined(RAW_ENCODING)
        transferRawImage(&g_tx_packet, img_size, g_scratch);
#endif
        transfer_time = xTaskGetTickCount() - tx_start;
        frame_tx++;
        if (frame_tx % 100 == 0)
            cpxPrintToConsole(LOG_TO_CRTP,
                "[INFO] frame #%u cap=%ums xfer=%ums\n",
                frame_tx, capture_time, transfer_time);
    }
}
...
void rxTask(void *parameters) {
    ...
    while (true) {
        cpxReceivePacketBlocking(CPX_F_WIFI_CTRL, &g_rx_packet);
        WiFiCTRLPacket_t *wifi_ctrl = (WiFiCTRLPacket_t *)g_rx_packet.data;

        switch (wifi_ctrl->cmd) {
...
void run(void) {
    ...
    x_task = xTaskCreate(ledTask, "LED_TASK", configMINIMAL_STACK_SIZE * 2,
    ...
    x_task = xTaskCreate(cameraTask, "CAMERA_TASK", configMINIMAL_STACK_SIZE * 8,
    ...
    x_task = xTaskCreate(rxTask, "RX_TASK", configMINIMAL_STACK_SIZE * 2, NULL,
