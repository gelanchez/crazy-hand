...
static void setup_nina_rtt_pin(pi_device_t *device) {
  ...
  pi_gpio_pin_notif_configure(device, CONFIG_NINA_GPIO_NINA_ACK,
                              PI_GPIO_NOTIF_RISE);
  pi_pad_set_function(CONFIG_NINA_GPIO_NINA_ACK_PAD,
                      CONFIG_NINA_GPIO_NINA_ACK_PAD_FUNC);

  // Set up interrupt
  uint32_t gpio_mask =
      (1 << (CONFIG_NINA_GPIO_NINA_ACK & PI_GPIO_NUM_MASK)); // create pin mask
  pi_gpio_callback_init(
      &cb_gpio, gpio_mask, vDataReadyISR,
      (void *)CONFIG_NINA_GPIO_NINA_ACK); // setup callback and handler
...
static void setup_gap8_rtt_pin(pi_device_t *device) {
  ...
  pi_pad_set_function(CONFIG_NINA_GPIO_NINA_NOTIF_PAD,
...
void com_task(void *parameters) {
  ...
  pi_gpio_pin_read(&nina_rtt_dev, CONFIG_NINA_GPIO_NINA_ACK,
                   &startupESPRTTValue);
  ...
  while (1) {
    ...
    // Check if we have more to send, if not then wait until we have or Nina
    // wants to send
    if (uxQueueMessagesWaiting(txq) == 0) {
      DEBUG_PRINTF("Waiting for action!\n");
      // Wait for either TXQ or RTT from Nina
      evBits = xEventGroupWaitBits(evGroup, NINA_RTT_BIT | TX_QUEUE_BIT,
    ...
    //  There's a risk that we've been emptying the queue while another package
    //  has been pushed and set the event bit again, which will trigger this
    //  loop again. To avoid one extra read (that's not needed) double check
    //  here.
    if ((evBits & NINA_RTT_BIT) == NINA_RTT_BIT || tx_buff.len > 0) {
      DEBUG_PRINTF("Initiating SPI tansfer\n");

      pi_spi_transfer(&spi_dev, &tx_buff, &rx_buff, INITIAL_TRANSFER_SIZE * 8,
      ...
      int sizeLeft = max(tx_len - INITIAL_TRANSFER_SIZE + 2,
                         rx_len - INITIAL_TRANSFER_SIZE + 2);
      ...
      pi_spi_transfer(&spi_dev, ((uint8_t *)&tx_buff) + INITIAL_TRANSFER_SIZE,
                      ((uint8_t *)&rx_buff) + INITIAL_TRANSFER_SIZE,
                      sizeLeft * 8, PI_SPI_LINES_SINGLE | PI_SPI_CS_AUTO);
...
void com_init() {
  ...
  BaseType_t xTask;
  xTask = xTaskCreate(com_task, "com_task", configMINIMAL_STACK_SIZE * 6, NULL,
...
void com_read(packet_t *p) { xQueueReceive(rxq, p, (TickType_t)portMAX_DELAY); }
