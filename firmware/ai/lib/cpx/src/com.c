...
static void setup_nina_rtt_pin(pi_device_t *device) {
    ...
    // Set up interrupt
    uint32_t gpio_mask = (1 << (CONFIG_NINA_GPIO_NINA_ACK &
                                PI_GPIO_NUM_MASK)); // create pin mask
...
void com_task(void *parameters) {
    ...
    while (1) {
        ...
        //  There's a risk that we've been emptying the queue while another
        //  package has been pushed and set the event bit again, which will
        //  trigger this loop again. To avoid one extra read (that's not needed)
        //  double check here.
        if ((evBits & NINA_RTT_BIT) == NINA_RTT_BIT || tx_buff.len > 0) {
            DEBUG_PRINTF("Initiating SPI tansfer\n");

            pi_spi_transfer(&spi_dev, &tx_buff, &rx_buff,
            ...
            if (rx_buff.len > 0) {
                if (xQueueSend(rxq, &rx_buff, (TickType_t)portMAX_DELAY) !=
                    pdPASS) {
            ...
            // Do not wait for Nina RTT to go low, we trigger on rising edge
            // anyway
        } else {
            // For debug
