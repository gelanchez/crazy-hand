...
bool cpxSendPacket(const CPXPacket_t *packet, uint32_t timeout) { return true; }
...
void cpxPrintToConsole(CPXConsoleTarget_t target, const char *fmt, ...) {
    if (xSemaphoreTake(xSemaphore, (TickType_t)portMAX_DELAY) == pdTRUE) {
        ...
        va_start(ap, fmt);
        len =
            vsnprintf((char *)consoleTx.data, sizeof(consoleTx.data), fmt, ap);
        va_end(ap);
...
void cpxInit(void) {
    ...
    BaseType_t rxTask =
        xTaskCreate(cpx_rx_task, "rx_task", configMINIMAL_STACK_SIZE * 2, NULL,
